...
- **Trust the CA**: `*.homelab.dev` is served with a self-signed cert (see [AGENT.md](../AGENT.md#tls)). Pull the CA's public cert out of the cluster and trust it in your OS/browser keychain — it's never the private key, so this is safe to do per-device:
  ```bash
